...
public abstract class Document {
	...
	protected int countSyllables(String word) {
		...
		int result = 0;
		if (wordLower.matches("[^aeiouy]*e")) {
			result = 1;
		} else {
			List<String> tks = Document.getTokensOfText("[aeiouy]+", wordLower);
			// System.out.println(tks);
			result = tks.size();

			// if (wordLower.endsWith("[r]e")) { //this doesn't work since
			// endsWith is not for regular expressions
			if (wordLower.matches("[a-z]*[^aeiouy]e")) {
	...
	public static List<String> getTokensOfText(String pattern, String text) {
		ArrayList<String> tokens = new ArrayList<String>();
		Pattern tokSplitter = Pattern.compile(pattern);
		Matcher m = tokSplitter.matcher(text);
	...
	public static void main(String[] args){
		System.out.println(getTokensOfText("[a-z]+|[()0-9]+", "one (1), two (2), three (3)"));
	}
	...
	public double getFleschScore() {
		...
		double result=206.835-1.015*((double)getNumWords()/getNumSentences())-84.6*((double)getNumSyllables()/getNumWords());
		return result;
	}
