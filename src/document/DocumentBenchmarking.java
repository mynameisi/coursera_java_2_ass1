...
public class DocumentBenchmarking {
	...
	public static void main(String[] args) {
		...
		// instructions.
		System.out.println("#words\tslow\tfast");// print the column names for
		...
		for (int numToCheck = start; numToCheck < numSteps * increment + start; numToCheck += increment) {
			...
			double time1 = 0, time2 = 0, t1 = 0, t1Temp = 0, t2 = 0, t2Temp = 0;
			Document d = null;
			for (int i = 0; i < trials; i++) {
				time1 = System.currentTimeMillis();
				d = new BasicDocument(s);
				d.getFleschScore();
				time2 = System.currentTimeMillis();
				t1Temp += (time2 - time1) / 1000;//==>add the time for each trail of a method
			}
			t1 = t1Temp / trials;//==>get the average time for this method
			
			System.out.print(t1 + "\t");
			for (int i = 0; i < trials; i++) {
				time1 = System.currentTimeMillis();
				d = new EfficientDocument(s);
				d.getFleschScore();
				time2 = System.currentTimeMillis();
				t2Temp += (time2 - time1) / 1000;
			}
			t2 = t2Temp / trials;
			System.out.print(t2 + "\n");
